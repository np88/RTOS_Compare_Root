...
\subsection{Bootzeit}
Die Bootzeit ist insofern ein Ausschlusskriterium für die Benutzung eines Betriebssystems, als dass es für manche Anwendungen nicht akzeptabel ist, mehr als einige Sekunden auf das Starten des Programms zu warten. Ein Beispiel hierfür ist das Automobil. Wenn man den Zündschlüssel umdreht und dann 15 Sekunden warten muss, bis das Auto angeht, ist das zu lang. Deswegen muss abgeschätzt werden, wie viel Bootzeit ein Betriebssystem in Anspruch nehmen darf und wie diese eventuell in einen akzeptablen Bereich gebracht werden kann.
\\\\Für die aktuelle Hardware gibt es mehrere Bootoptionen:
\begin{itemize}
	\item Booten über JTag
	\item Booten von der SD Karte
	\item Booten aus dem QSPI Nor-Flash Speicher
\end{itemize}

Die schnellste dieser Optionen ist das Booten vom QSPI, deswegen wird hauptsächlich dieses für den Vergleich herangezogen. Es ist allerdings einfacher, verschiedene Bootkonfigurationen von der SD Karte zu testen. Deswegen werden verschiedene Bootkonfigurationen zuerst auf der SD Karte ausprobiert, um zu sehen, ob es sich lohnt, den QSPI-Flash und den Bootloader dafür umzuprogrammieren. Bei dem Messen der Bootzeiten wird immer eine PL in das Projekt integriert. Der Boot-Prozess von der SD-Karte ist noch nicht ausgereift und dauert mit einem FSBL, der Logik enthält, sehr lange.  

\subsubsection{FreeRTOS}
Der Bootvorgang für FreeRTOS besteht für das Laden von einem Flash-Medium darin, dass zuerst der FSBL geladen wird. Dieser konfiguriert die Hardware (PL) und startet das Betriebssystem. 
\subsubsection{Linux}
Der Bootvorgang von Linux ist deutlich komplexer. Auch hier wird zunächst ein FSBL gestartet, der die Hardware konfiguriert. Optional kann das Aufspielen der Logik erfolgen, dieses kann aber auch später durch das Betriebssystem erfolgen. Der FSBL lädt dann einen Bootloader für das Betriebssystem. Dieser ist für das Laden und Initialisieren des Betriebssystems und optional für das Root-Filesystem verantwortlich. An dieser Stelle muss mindestens das Betriebssystem-Image in den RAM geladen werden. Außerdem kann noch ein Device-Tree zur Systemkonfiguration und ein Ram-Image von dem Bootloader in das RAM geladen werden. Als letztes muss das Betriebssystem laden. Es ist sinnvoll, das System so aufzusetzen, dass so wenig wie möglich geladen werden muss, da die Zeit für den Kopiervorgang linear zu der Größe der Datei ansteigt. Dafür wurde der Standard Xilinx-Kernel mit einer Größe von etwa 2,9 MB auf eine Größe von 1,9 MB gebracht, indem nicht benötigte Treiber und Features nicht mitcompiliert wurden. Der Devicetree hat eine Größe von ca 5 KB.
\paragraph{Versuch 1}
 Bei dem Standard-Bootvorgang von Xilinx werden ein Linux-Image, ein Ramdisk-Image und ein Devicetree vom Flash in das RAM geladen. Die Standard-Flash-Partitionen sind so ausgelegt, dass ein Boot-Image mit PL nicht darin Platz findet. Deswegen wird die PL über das Betriebssystem beschrieben. Das Image ist ca 5,1 MB groß und verändert nach dem Komprimieren seine Größe nur unwesentlich, auch wenn ein 4 MB großes PL file hinzugefügt wurde. Insgesamt müssen also ca. 7 MB in das RAM geladen werden. Der Bootvorgang dauert für diese Konfiguration 13 Sekunden. Das Laden des Bootloaders dauert etwa eine Sekunde sowie das Konfigurieren des Betriebssystem. Das heißt, dass das Laden in den QSPI-Flash etwa 11 Sekunden dauert. Dieses ist nicht akzeptabel und sollte optimiert werden.
\paragraph{Versuch 2}
Bei dem zweiten Versuch wurde nach einem Ansatz gesucht, bei dem das Root File System nicht in den QSPI Flash geladen werden muss. Eine Möglichkeit ist der Einsatz eines UBIFS. Das UBIFS hat wesentliche Vorteile gegenüber einem Ramimage: Es ist schreibbar, d.h., die Änderungen, die gemacht werden, bleiben auch nach dem Power-Off erhalten. Außerdem wird nicht direkt das gesamte File System geladen, sondern nur die Dateien, die gerade gebraucht werden. Deswegen wird es auch nicht vom Boot-Loader bearbeitet, sondern vom Betriebssystem selber initialisiert. Die Gesamtbootzeit liegt nun bei 7,5 Sekunden. Das Laden des Linux-Images dauert ca. 3 Sekunden. Danach kommt das Laden und die Konfiguration des Betriebssystems. Dieses verlängert sich auf etwa 3 Sekunden. Insgesamt bringt das Ändern des Filesystems auch eine Zeiteinsparung von 50 Prozent.

\subsubsection{Weitere Möglichkeiten des Zeiteinsparens}
Der Versuchsaufbau war erst ein Anfang, um die Bootzeit zu verkürzen. Es gibt noch weitere Möglichkeiten, wie man die Zeit senken kann.
\paragraph{Anderer Bootloader}
Es kann ein anderer Bootloader benutzt werden, der zum Beispiel das Betriebssystem direkt aus dem Speicher lädt, ohne es vorher in den RAM zu kopieren. Dieses könnte die Konfigurationszeit des Betriebssystems geringfügig verlängern, würde aber die 3 Sekunden Kopierzeit einsparen.
\paragraph{Kleinere Dateien}
Das Kernel-Image und das Filesystem könnten auf das minimal benötigte reduziert werden, wodurch die Zeit für den Kopieraufwand weiter sinken würde.
