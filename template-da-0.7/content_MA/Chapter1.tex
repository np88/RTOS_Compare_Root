...
A typical set up are separate level 1 data and instruction caches for each \ac{CPU} and a shared level 2 cache for both data and instructions for all \ac{CPUS}.
...
Because of efficiency considerations, the data blocks are managed in pages of consequent data.
The pages used by the real-time application could be flushed from the \ac{RAM} which causes hardly predicable reloading of pages.
Yet, there is the possibility to to lock specific pages for critical data or applications in \ac{RAM}.
This application should be used carefully because it might increase the run time of one application but decrease the over-all system performance.   
...
\begin{enumerate}
	\item Deterministic part $ t_{det} = t_{app} + t_{os} + t_{isr} $: 		
		...
		\end{enumerate}	
  \item Not deterministic part, jitter $ t_{jitter} = t_{tick} + t_{int} + t_{cache} + t_{daem} $:	
		\begin{enumerate}
