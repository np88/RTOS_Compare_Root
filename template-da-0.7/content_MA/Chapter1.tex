...
On one hand, caches increase the performance of an application significantly by reducing slower memory accesses to the \ac{RAM}. 
...
These latencies are implementation specific and can be measured.
\par
...
Yet, there is the possibility to lock specific pages for critical data or applications in \ac{RAM}.
...
Usually, all raised softirqs are handled by the do\_softirq function, but when too many softirqs are pending to be executed, they are moved to the ksoftirqd thread which is scheduled when the \ac{OS} is not busy.
