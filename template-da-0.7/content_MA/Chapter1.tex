	...
	\item Deterministic part $ t_{det} = t_{app} + t_{os} + t_{isr} $: 		
		\begin{enumerate}
			...
			\item Execution time for \ac{OS} related features where $ n_{os} $ is the number of \ac{OS} 
						...
						$ t_{os} = \sum\limits_{i=1}^{n_{os}} {x_i * t_{os}^{i}} $
				\item Execution time for \acp{ISR} belonging to the application $ n_{isr} $ is the number of 
						...
						$ t_{isr} = \sum\limits_{i=1}^{n_{isr}} {x_i * t_{isr}^{i}} $						
		\end{enumerate}	
  \item Not deterministic part $ t_{non-det} = t_{tick} + t_{int} + t_{cache} + t_{deam} $:	
		\begin{enumerate}
			...
			\item Delay caused by \ac{OS} deamon processes where $ n_{deam} $ is the number of deamon processes 
						$ t_{deam} = \sum\limits_{i=1}^{n_{deam}} {t_{deam}^{i}} $
		\end{enumerate}	
\end{enumerate}
 
$ t_{non-det} $ is also dependent on the probability of the single delays.
As only the worst case execution time is of interest for further consideration, the formula stated above can be used for the calculation of $ t_{non-det} $ without modification.     
